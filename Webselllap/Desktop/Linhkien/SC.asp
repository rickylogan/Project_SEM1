...
<%
Dim LINHKIEN_SOUNDCARD
Dim LINHKIEN_SOUNDCARD_cmd
Dim LINHKIEN_SOUNDCARD_numRows

Set LINHKIEN_SOUNDCARD_cmd = Server.CreateObject ("ADODB.Command")
LINHKIEN_SOUNDCARD_cmd.ActiveConnection = MM_Connection_STRING
LINHKIEN_SOUNDCARD_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 3 and TenSP like '%sound card% ' or TenSP like '%sound%'" 
LINHKIEN_SOUNDCARD_cmd.Prepared = true

Set LINHKIEN_SOUNDCARD = LINHKIEN_SOUNDCARD_cmd.Execute
LINHKIEN_SOUNDCARD_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
LINHKIEN_SOUNDCARD_numRows = LINHKIEN_SOUNDCARD_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim LINHKIEN_SOUNDCARD_total
Dim LINHKIEN_SOUNDCARD_first
Dim LINHKIEN_SOUNDCARD_last

' set the record count
LINHKIEN_SOUNDCARD_total = LINHKIEN_SOUNDCARD.RecordCount

' set the number of rows displayed on this page
If (LINHKIEN_SOUNDCARD_numRows < 0) Then
  LINHKIEN_SOUNDCARD_numRows = LINHKIEN_SOUNDCARD_total
Elseif (LINHKIEN_SOUNDCARD_numRows = 0) Then
  LINHKIEN_SOUNDCARD_numRows = 1
End If

' set the first and last displayed record
LINHKIEN_SOUNDCARD_first = 1
LINHKIEN_SOUNDCARD_last  = LINHKIEN_SOUNDCARD_first + LINHKIEN_SOUNDCARD_numRows - 1

' if we have the correct record count, check the other stats
If (LINHKIEN_SOUNDCARD_total <> -1) Then
  If (LINHKIEN_SOUNDCARD_first > LINHKIEN_SOUNDCARD_total) Then
    LINHKIEN_SOUNDCARD_first = LINHKIEN_SOUNDCARD_total
  End If
  If (LINHKIEN_SOUNDCARD_last > LINHKIEN_SOUNDCARD_total) Then
    LINHKIEN_SOUNDCARD_last = LINHKIEN_SOUNDCARD_total
  End If
  If (LINHKIEN_SOUNDCARD_numRows > LINHKIEN_SOUNDCARD_total) Then
    LINHKIEN_SOUNDCARD_numRows = LINHKIEN_SOUNDCARD_total
  End If
End If
%>
...
Set MM_rs    = LINHKIEN_SOUNDCARD
MM_rsCount   = LINHKIEN_SOUNDCARD_total
MM_size      = LINHKIEN_SOUNDCARD_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
LINHKIEN_SOUNDCARD_first = MM_offset + 1
LINHKIEN_SOUNDCARD_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (LINHKIEN_SOUNDCARD_first > MM_rsCount) Then
    LINHKIEN_SOUNDCARD_first = MM_rsCount
  End If
  If (LINHKIEN_SOUNDCARD_last > MM_rsCount) Then
    LINHKIEN_SOUNDCARD_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM SOUND CARD MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT LINHKIEN_SOUNDCARD.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(LINHKIEN_SOUNDCARD.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(LINHKIEN_SOUNDCARD.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(LINHKIEN_SOUNDCARD.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(LINHKIEN_SOUNDCARD.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspLaptop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(LINHKIEN_SOUNDCARD.Fields.Item("MaSP").Value)%>">
      <label>
...
<%
LINHKIEN_SOUNDCARD.Close()
Set LINHKIEN_SOUNDCARD = Nothing
%>
