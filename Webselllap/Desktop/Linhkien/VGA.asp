...
<%
Dim LINHKIEN_VGA
Dim LINHKIEN_VGA_cmd
Dim LINHKIEN_VGA_numRows

Set LINHKIEN_VGA_cmd = Server.CreateObject ("ADODB.Command")
LINHKIEN_VGA_cmd.ActiveConnection = MM_Connection_STRING
LINHKIEN_VGA_cmd.CommandText = "SELECT * FROM dbo.SanPham WHERE Tinhtrang = 1 and MaLoai = 3 and TenSP like '%vga%'" 
LINHKIEN_VGA_cmd.Prepared = true

Set LINHKIEN_VGA = LINHKIEN_VGA_cmd.Execute
LINHKIEN_VGA_numRows = 0
%>
<%
...
Repeat1__numRows = 9
Repeat1__index = 0
LINHKIEN_VGA_numRows = LINHKIEN_VGA_numRows + Repeat1__numRows
%>
<%
'  *** Recordset Stats, Move To Record, and Go To Record: declare stats variables

Dim LINHKIEN_VGA_total
Dim LINHKIEN_VGA_first
Dim LINHKIEN_VGA_last

' set the record count
LINHKIEN_VGA_total = LINHKIEN_VGA.RecordCount

' set the number of rows displayed on this page
If (LINHKIEN_VGA_numRows < 0) Then
  LINHKIEN_VGA_numRows = LINHKIEN_VGA_total
Elseif (LINHKIEN_VGA_numRows = 0) Then
  LINHKIEN_VGA_numRows = 1
End If

' set the first and last displayed record
LINHKIEN_VGA_first = 1
LINHKIEN_VGA_last  = LINHKIEN_VGA_first + LINHKIEN_VGA_numRows - 1

' if we have the correct record count, check the other stats
If (LINHKIEN_VGA_total <> -1) Then
  If (LINHKIEN_VGA_first > LINHKIEN_VGA_total) Then
    LINHKIEN_VGA_first = LINHKIEN_VGA_total
  End If
  If (LINHKIEN_VGA_last > LINHKIEN_VGA_total) Then
    LINHKIEN_VGA_last = LINHKIEN_VGA_total
  End If
  If (LINHKIEN_VGA_numRows > LINHKIEN_VGA_total) Then
    LINHKIEN_VGA_numRows = LINHKIEN_VGA_total
  End If
End If
%>
...
Set MM_rs    = LINHKIEN_VGA
MM_rsCount   = LINHKIEN_VGA_total
MM_size      = LINHKIEN_VGA_numRows
MM_uniqueCol = ""
MM_paramName = ""
MM_offset = 0
MM_atTotal = false
...
' set the first and last displayed record
LINHKIEN_VGA_first = MM_offset + 1
LINHKIEN_VGA_last  = MM_offset + MM_size

If (MM_rsCount <> -1) Then
  If (LINHKIEN_VGA_first > MM_rsCount) Then
    LINHKIEN_VGA_first = MM_rsCount
  End If
  If (LINHKIEN_VGA_last > MM_rsCount) Then
    LINHKIEN_VGA_last = MM_rsCount
  End If
End If
	 	...
	 	<div class="wrap">
	 		<div class="pages">
				<div class="cont1 span_2_of_g1">
				  <p>CÁC SẢN PHẨM VGA MỚI NHẤT</p>
				  <p>&nbsp;</p>
                  <% 
While ((Repeat1__numRows <> 0) AND (NOT LINHKIEN_VGA.EOF)) 
%>
  <div class="oneItem">
    <p><img src="<%=(LINHKIEN_VGA.Fields.Item("HinhAnh").Value)%>" alt="" width="225" height="150">Sản phẩm: <%=(LINHKIEN_VGA.Fields.Item("TenSP").Value)%></p>
    <p>Giá: <%=(LINHKIEN_VGA.Fields.Item("Gia").Value)%></p>
    <p>Hiện còn <%=(LINHKIEN_VGA.Fields.Item("SoLuong").Value)%> sản phẩm</p>
    <p>&nbsp;</p>
    <form name="form1" method="post" action="ctspLaptop.asp">
      <input name="MaSP" type="hidden" id="MaSP" value="<%=(LINHKIEN_VGA.Fields.Item("MaSP").Value)%>">
      <label>
